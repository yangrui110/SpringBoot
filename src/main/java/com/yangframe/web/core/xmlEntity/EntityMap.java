...
import com.yangframe.config.datasource.dynamic.InfoOfDruidDataSourceConfig;
...
public class EntityMap {
    ...
    private static String makeJoinString(Element element){
        ...
        String quote = "";
        if(DataSourceType.ORACLE.equals(mainTable.getInfoOfEntity().getConfig().getSourceType())){
            quote="\"";
        }
        //获取view-link的出现顺序
        List<String> sortViewLink = getSortViewLink(element);
        for(String m : sortViewLink){
    ...
    private static String makeOrderBy(List<COrderBy> orderBy,Map<String,ColumnProperty> columns){
        ...
        columns.forEach((k,v)->{
            orderBy.forEach((m)->{
                if(m.getNames().equals(v.getAlias().replaceAll("\"","" ))){
    ...
    private static Map<String,JoinRelation> getJoinRelation(Element element){
        ...
        Map<String, ColumnProperty> allColumns = EntityMap.getNoExcludeColumns(element);
        List<Element> elements = element.elements();
    ...
    public static Map<String,ColumnProperty> readColumnEntity(Element element){
        ...
        elements.forEach((el)->{
            if(el.getName().equals("column")) {
                ...
                if (name == null)
                    throw new BaseException(304, "该标签" + el.getName() + "的alias属性不能为空");
    ...
    /**
     * 获取所有的主键
     * 主键通过primary-key强制定义主键定义，否则就会返回主表的主键
     * */
        ...
        if (element != null) {
            ...
            elements.forEach((k) -> {
                if ("primary-key".equals(k.getName())) {
                    String name = k.attributeValue("alias");
    ...
    private static Map<String,ColumnProperty> getColumnMap(Element element){
        ...
        elements.forEach((el)->{
            if("column".equals(el.getName())) {
                ...
                ColumnProperty property=new ColumnProperty();
                property.setColumn(name==null?humpToUnderline(alias):name);
                property.setAlias(alias);
                property.setTableMemberAlias("");
